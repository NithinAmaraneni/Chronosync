...
export default function StudentTimetablePage() {
  ...
  const bySemester = flatSlots.reduce((acc: Record<string, Record<string, any[]>>, slot: any) => {
    const semKey = slot.subject?.semester || 'Unassigned Semester';
    if (!acc[semKey]) {
      acc[semKey] = {};
      days.forEach(d => acc[semKey][d] = []);
    }
    if (acc[semKey][slot.day]) acc[semKey][slot.day].push(slot);
    return acc;
  }, {});
      ...
      {!hasSlots ? (
        ...
        <div className="ds-card ds-fade-in" style={{ padding: '1rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {Object.keys(bySemester).sort(sortSections).map(semKey => (
              <section key={semKey} style={{ borderTop: '1px solid rgba(0,0,0,0.06)', paddingTop: '0.85rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '0.75rem' }}>
                  <span className="ds-badge ds-badge-blue">{semKey}</span>
                  <span style={{ fontSize: '0.76rem', color: '#9a7b6a' }}>{days.reduce((sum, day) => sum + bySemester[semKey][day].length, 0)} classes</span>
                </div>
                <div className="ds-timetable">
                  {days.map(day => (
                    <div key={day}>
                      <div className="ds-tt-day">{day.slice(0, 3)}</div>
                      {bySemester[semKey][day].length === 0 ? (
                        <div className="ds-tt-empty">Free</div>
                      ) : (
                        bySemester[semKey][day].map((slot: any) => (
